...
- [ ] **Every** `server.json` version matches the tag: the top-level `version`, each package's `version`, and the image tag inside the OCI package's `identifier` (the registry rejects a `version` key on an OCI package, so for that entry the tag *is* the version). `scripts/prepare-release.py` moves all of them. Note what does NOT check them: `scripts/validate-release.py` has no version check at all, and release.yml's "Validate version consistency" step covers `pyproject.toml` and `server.json` but not `uv.lock`. A stale image tag would publish a registry entry pointing at the previous release's container, so confirm this one by eye.
