...
- Client certificates are stored under `~/.gemini/certs/` by default. One that already covers a host/port/path scope is attached automatically; nothing in the MCP tool surface creates one, so a capsule answering status 60 cannot be satisfied through these tools.
